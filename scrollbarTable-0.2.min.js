...
return width;}());var cols=[];$t.find('tr:first th,tr:first td').each(function(){cols.push($(this).width());});$t.css('table-layout','fixed');var $firstRow=$t.clone();$firstRow.find('tr:not(:first)').remove();$t.find('tr:first').remove();$t.before($firstRow);$firstRow.find('tr:first th,tr:first td').each(function(i){$(this).attr('width',cols[i]);});$t.find('tr:first th,tr:first td').each(function(i){$(this).attr('width',cols[i]);});var $wrap=$('<div>');if(h<o.height)o.height=$h;$wrap.css({width:w,height:o.height,overflow:'auto'});$t.wrap($wrap);})};}(jQuery));
